...
import com.example.face_recognition_attendance_app.Activities.FaceRecognitionActivity;
...
public class FaceRecognitionProcessor extends VisionBaseProcessor<List<Face>> {
    ...
    public void fetchDataFromFirestore() {
        ...
        CollectionReference faceVectorCollection = db.collection("Users");

        faceVectorCollection
                .get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    for (QueryDocumentSnapshot document : queryDocumentSnapshots) {
                        String userId = document.getId();
                        if (document.contains("face_vector")) {
                            String faceVectorString = (String) document.get("face_vector");

                            float[] faceVectorArray = convertStringToFloatArray(faceVectorString);

                            Person person = new Person(userId, faceVectorArray);
                            recognisedFaceList.add(person);
                        } else {
                            Log.d(TAG, "User " + userId + " does not have a face_vector");
                        }
                    }
                })
                .addOnFailureListener(e -> {
                    Log.w(TAG, "Error fetching documents from Firestore", e);
                });
    }
...
//                            }
    ...
    public static float[] convertStringToFloatArray(String stringValue) {
